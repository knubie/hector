module Hector
  class Channel
    attr_reader :name, :topic, :user_sessions, :ops, :created_at, :modes, :invites, :op_nicks
    ...
    def join(session)
      ...
      if user_sessions.empty? || op_nicks.include?(session.name)
        set_op(session)
      end
