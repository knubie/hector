  ...
  module Commands
    module Whois
      ...
      def respond_to_whois_for(destination, session)
        ...
        respond_with("319", destination, session.nickname, :text => session.channels.map { |channel| channel.name unless channel.secret? }.reject { |i| i.nil? }.join(" ")) unless session.channels.empty?
